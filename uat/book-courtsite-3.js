...
// Format: "YYYY-MM-DD"
const BOOK_DATE      = "2026-06-05";   // ← change each time you run
const START_HOUR     = 13;             // 13 = 1 PM (1:00 PM)
const DURATION_HOURS = 2;              // 1 hour session
...
function isUnavailable(data) {
  const t = data?.startCourtsiteCheckout?.__typename || "";
  return t.includes("Unavailable") || t.includes("unavailable");
}
...
async function main() {
  ...
  for (let attempt = 1; attempt <= 3; attempt++) {
    try {
      ...
      // Check if slot is explicitly unavailable
      if (isUnavailable(checkoutData)) {
        throw new Error(`Slot unavailable: ${checkoutData?.startCourtsiteCheckout?.message || "unknown reason"}`);
      }

      const workflowID = extractWorkflowID(checkoutData);
      if (!workflowID) {
        console.error("\n⚠️  Could not find workflowID in response above.");
        console.error("Unexpected response shape — raw data printed above.");
        process.exit(1);
      }
